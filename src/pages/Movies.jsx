...
const Movies = () => {
  ...
  const fetchMovies = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=88613ddc587dbef4018d7fc2cf69285e&page=${page}`);
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-10 gap-y-7 gap-x-4">
          {movies.map((movie) => (
            <div className="relative">
              <Link to={`/movie/${movie?.id}`} key={movie.id} className="cursor-pointer relative hover:text-hvr flex flex-col gap-y-2">
                <LazyLoadImage className="w-full h-full rounded-xl" src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt={movie.title} />
                <h1 className="h-16">{movie.title}</h1>
              </Link>
